...
public class Rider : Form
{
    ...
    private async Task LoadExistingOrdersAsync()
    {
        try
        {
            ...
            {
                _orders.Clear();
                _orders.AddRange(orders.OrderByDescending(o => o.CreatedAt));
                RenderOrders();
            });
        }
        catch
    ...
    private async Task ConnectSignalRAsync()
    {
        ...
        _connection.On<int, string, string, List<string>, int, string>(
            ...
            (orderId, customer, restaurantName, items, total, status) =>
            {
                Invoke(() =>
                {
                    _orders.Insert(0, new OrderSummary(
                        orderId,
                        customer,
                        restaurantName,
                        status,
    ...
    private Panel CreateOrderCard(OrderSummary order)
    {
        ...
        btnDelivering.Click += async (s, e) =>
            await UpdateOrderStatusAsync(order.Id, "Delivering");
        ...
        {
            Text = $"🕐 {order.CreatedAt.ToLocalTime():dd/MM HH:mm}",
